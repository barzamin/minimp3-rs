...
fn decode_frame(ctx: &mut minimp3_sys::mp3dec_t, mp3_file: &[u8], pcm: &mut [i16], frame_info: &mut minimp3_sys::mp3dec_frame_info_t) -> Option<usize> {
    unsafe {
        let samples = minimp3_sys::mp3dec_decode_frame(ctx, mp3_file.as_ptr(), mp3_file.len() as _, pcm.as_mut_ptr(), frame_info);
